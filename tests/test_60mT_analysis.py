#!/usr/bin/env python3
"""
分析 60mT 平行磁場條件下的 CPR (Current-Phase Relation) 實驗
樣品 003-2，磁場角度掃描分析
"""
...
def analyze_parameters_with_angles(df, angle_file_map, experiment_info):
    """分析參數統計（帶角度資訊）"""
    if df.empty:
        print("❌ 沒有數據可分析")
        return
    ...
    print(f"\n📈 約瑟夫森結參數統計 ({experiment_info['condition']}):")
    print("-" * 60)
...
def display_detailed_results_with_angles(df, angle_file_map, experiment_info):
    """顯示詳細結果（帶角度資訊）"""
    if df.empty:
        return
    ...
    print(f"\n📋 磁場角度掃描詳細結果:")
    print("=" * 120)
    print(f"樣品: {experiment_info['sample_id']}, 條件: {experiment_info['condition']} ({experiment_info['description']})")
    print()
...
def create_comparison_plots_with_angles(df, angle_file_map, experiment_info, output_dir):
    ...
    for i, param in enumerate(parameters):
        ...
        if len(values) > 0:
            # 用線條和點連接，顯示角度依賴性
            ax.plot(angles_for_param, values, 'o-', linewidth=2, markersize=6, color='red')
    ...
    plt.tight_layout()
    plt.savefig(output_path / 'parameters_vs_angle_60mT.png', dpi=300, bbox_inches='tight')
    print(f"✓ 60mT 參數角度依賴性圖已保存: {output_path / 'parameters_vs_angle_60mT.png'}")
    ...
    for i, stat in enumerate(statistics):
        ...
        if len(values) > 0:
            ax.plot(angles_for_stat, values, 'o-', linewidth=2, markersize=6, color='red')
    ...
    plt.tight_layout()
    plt.savefig(output_path / 'fitting_quality_vs_angle_60mT.png', dpi=300, bbox_inches='tight')
    print(f"✓ 60mT 擬合品質角度依賴性圖已保存: {output_path / 'fitting_quality_vs_angle_60mT.png'}")
...
def main():
    ...
    print("🔬 Current-Phase Relation 參數角度依賴性分析 (60mT 條件)")
    print("=" * 80)
    ...
    # 分析參數（傳入角度資訊）
    success_df = analyze_parameters_with_angles(target_df, angle_file_map, experiment_info)
    
    # 顯示詳細結果（傳入角度資訊）
    display_detailed_results_with_angles(target_df, angle_file_map, experiment_info)
    
    # 創建比較圖表（傳入實驗資訊）
    output_dir = "/Users/albert-mac/Code/GitHub/CPR/output/parameter_analysis"
    final_df = create_comparison_plots_with_angles(target_df, angle_file_map, experiment_info, output_dir)
